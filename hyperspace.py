...
"""
    Generates a 3d projection of a n-dimensional objects
...
highlight_cell       = True
export_frames        = False
connect_verticies    = True

def hypercube(dims):
    c = []
    for i in range(2 ** dims):
        binary = bin(i)[2:]
        binary = binary.rjust(dims, "0")
...
if dimensions < 1: raise ValueError("Too few dimensions")

# Globals
width, height = 1024, 650
dims          = dimensions # dimensions of the cube
                                     # there are 2^n verticies in an n dimensional cube
scale         = 5 ** dims * 1.5
framecount    = 0

## Define Shape ##
points = hypersphere(dims)#hypercube(dims)
